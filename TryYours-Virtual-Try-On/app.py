...
def upload_selfie():
    try:
        ...
        response_data = {'status': 'success', 'message': '사진이 성공적으로 업로드되었습니다.'}
...
def result():
  ...
  return render_template('result.html', cloth_param=cloth_param)
...
def chatbot_Answer():
    ...
    if request.method == 'POST':
        ...
        token = 'ewgDTpMA2eV5Hu_NG52N-qhalWFw_fRhsb2_AD3du_PfzgN22LT4Vc3CzyTP0_oWXfskag.'
        bard = Bard(token=token)
        response = bard.get_answer(f"{user_question}")['content']
